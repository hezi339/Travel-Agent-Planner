...
TOOL_MAP = {
    ...
    "search_travel_notes": search_travel_notes,
...
def execute_tool(tool_name: str, tool_args: list):
    ...
    try:
        ...
        if tool_name == "checklist_categories":
            ...
            # query 必填；city 可选第二参数

            query = tool_args[0] if tool_args else ""

            city = tool_args[1] if len(tool_args) > 1 else ""

            return str(search_travel_notes(query, city))

        elif tool_name == "recommend_clothing" and len(tool_args) >= 2:
